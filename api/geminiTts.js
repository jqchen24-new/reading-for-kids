...
const TRANSIENT_GEMINI_HTTP = new Set([429, 500, 503])
...
export async function synthesizeSpeechToWav(text) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    let res
    let rawText = ''
    for (let httpTry = 0; httpTry < GEMINI_HTTP_RETRIES; httpTry++) {
      res = await fetch(url, {
        method: 'POST',
        headers: {
      ...
      if (transient && httpTry < GEMINI_HTTP_RETRIES - 1) {
        await sleep(200 * 2 ** httpTry)
        continue
      }
